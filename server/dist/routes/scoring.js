    ...
    try {
        ...
        if (scores && scores.length > 0) {
            await logAdminAction(req, AUDIT_ACTIONS.SCORING_EVENT_ADDED, 'episode', episodeId, {
                scores_count: scores.filter((s) => s.quantity > 0).length,
                castaways: [...new Set(scores.map((s) => s.castaway_id))],
            });
        }
        res.json(result.data);
    }
...
router.get('/:id/scoring/audit', requireAdmin, async (req, res) => {
    try {
        const episodeId = req.params.id;
...
router.get('/:id/scoring/preview', requireAdmin, async (req, res) => {
    try {
        const episodeId = req.params.id;
        // Get episode info
      ...
      `)
        ...
        for (const score of draftScores || []) {
            ...
            const cname = score.castaways?.name || 'Unknown';
      ...
      `)
        ...
        for (const pick of picks || []) {
            ...
            const leagueName = pick.leagues?.name;
