...
export async function startScoringSession(
  episodeId: string
): Promise<{ data?: StartSessionResult; error?: string; status?: number }> {
...
export async function saveScores(
  episodeId: string,
  userId: string,
  scores: SaveScoresInput[]
): Promise<{ data?: { saved: number }; error?: string; status?: number }> {
  ...
  for (const castawayId of castawayIds) {
    ...
    for (const score of castawayScores) {
      ...
      if (!scoring_rule_id || quantity <= 0) continue;
...
export async function getScoringStatus(
  episodeId: string
): Promise<{ data?: ScoringStatus; error?: string; status?: number }> {
  const { data: episode } = await supabase
    .from('episodes')
    .select('id, is_scored')
    .eq('id', episodeId)
  ...
  const { data: completeness, error: rpcError } = await supabaseAdmin.rpc(
...
export async function finalizeScoring(
  episodeId: string,
  userId: string
): Promise<{ data?: FinalizeResult; error?: string; status?: number }> {
...
export async function getEpisodeScores(
  episodeId: string
): Promise<{ data?: EpisodeScores; error?: string; status?: number }> {
 ...
 */
export async function getCastawayScores(
  episodeId: string,
  castawayId: string
): Promise<{ data?: CastawayScores; error?: string; status?: number }> {
  // Check if scoring is finalized
  const { data: session } = await supabase
    .from('scoring_sessions')
    .select('status')
...
export async function recalculateStandings(
  seasonId: string
): Promise<{ data?: RecalculateResult; error?: string; status?: number }> {
  if (!seasonId) {
