...
export async function submitPick(
  ...
  castawayId: string,
  episodeId: string
): Promise<{ data?: { pick: any }; error?: string; status?: number }> {
 ...
 * Send pick confirmation email (fire and forget)
 */
async function sendPickConfirmationEmail(
  userId: string,
  castawayId: string,
  leagueId: string,
  episode: any
 ...
 */
export async function getCurrentPickStatus(
  leagueId: string,
  userId: string
): Promise<{ data?: CurrentPickStatus; error?: string; status?: number }> {
  const { data: league } = await supabase
    .from('leagues')
    .select('season_id')
    .eq('id', leagueId)
 ...
 */
export async function lockPicks(): Promise<{
 ...
 */
export async function autoFillPicks(): Promise<{
  data?: AutoFillResult;
  error?: string;
  status?: number;
}> {
  ...
  const autoPicks: AutoPickResult[] = [];
  ...
  for (const episode of episodes) {
    ...
    for (const league of leagues) {
      ...
      for (const member of missingUsers) {
        ...
        const activeCastaway = roster?.find((r: any) => r.castaways?.status === 'active');
 ...
 * Send auto-pick alert emails (fire and forget)
 */
async function sendAutoPickEmails(autoPicks: AutoPickResult[]): Promise<void> {
