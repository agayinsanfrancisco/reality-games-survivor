...
router.get('/nail-biter-leagues', async (_req: Request, res: Response) => {
  try {
    ...
    const leagueMembers: Record<string, Set<string>> = {};
    (members || []).forEach((m) => {
      if (!leagueMembers[m.league_id]) {
        leagueMembers[m.league_id] = new Set();
      }
      leagueMembers[m.league_id].add(m.user_id);
    });

    // Get all weekly picks
    const { data: picks, error: picksError } = await supabase
      .from('weekly_picks')
      .select('user_id, league_id, episode_id, castaway_id')
      .not('castaway_id', 'is', null);
    
    if (picksError) throw picksError;

    // Calculate weekly scores per user per league
    // Structure: league -> episode -> user -> points
    const leagueEpisodeUserPoints: Record<string, Record<string, Record<string, number>>> = {};

    (picks || []).forEach((pick: any) => {
      if (!pick.castaway_id || !pick.league_id) return;
      
      const points = castawayEpisodePoints[pick.episode_id]?.[pick.castaway_id] || 0;

      if (!leagueEpisodeUserPoints[pick.league_id]) {
        leagueEpisodeUserPoints[pick.league_id] = {};
      }
      if (!leagueEpisodeUserPoints[pick.league_id][pick.episode_id]) {
        leagueEpisodeUserPoints[pick.league_id][pick.episode_id] = {};
      }
      if (!leagueEpisodeUserPoints[pick.league_id][pick.episode_id][pick.user_id]) {
        leagueEpisodeUserPoints[pick.league_id][pick.episode_id][pick.user_id] = 0;
      }
      leagueEpisodeUserPoints[pick.league_id][pick.episode_id][pick.user_id] += points;
    });

    // For each league, count nail-biter weeks (margin <= 10 points)
    const leagueStats = (leagues || []).map((league) => {
      const episodeData = leagueEpisodeUserPoints[league.id] || {};
      let nailBiterWeeks = 0;
      let totalWeeks = 0;
      let closestMargin = Infinity;

      Object.values(episodeData).forEach((userPoints) => {
        const scores = Object.values(userPoints);
    ...
    const leaderboard = leagueStats
      .filter((l) => l.total_weeks >= 1)
...
router.get('/benchwarmer-regret', async (_req: Request, res: Response) => {
  try {
    ...
    // Get all rosters to know who owns which castaways
    const { data: rosters, error: rostersError } = await supabase
      .from('rosters')
      .select(`
        user_id,
    ...
    // Build user-league-castaway ownership map
    const userLeagueCastaways: Record<string, Record<string, Set<string>>> = {};
    (rosters || []).forEach((r: any) => {
      if (!r.castaway_id) return;
      const key = r.user_id;
      if (!userLeagueCastaways[key]) {
        userLeagueCastaways[key] = {};
      }
      if (!userLeagueCastaways[key][r.league_id]) {
        userLeagueCastaways[key][r.league_id] = new Set();
      }
      userLeagueCastaways[key][r.league_id].add(r.castaway_id);
    });

    // Get all weekly picks (to know which castaways were started)
    const { data: picks, error: picksError } = await supabase
      .from('weekly_picks')
      .select('user_id, league_id, episode_id, castaway_id')
      .not('castaway_id', 'is', null);
    ...
    // Track started castaways per user-league-episode
    const startedCastaways: Record<string, Record<string, Record<string, Set<string>>>> = {};
    (picks || []).forEach((pick: any) => {
      if (!pick.castaway_id) return;
      if (!startedCastaways[pick.user_id]) {
        startedCastaways[pick.user_id] = {};
      }
      if (!startedCastaways[pick.user_id][pick.league_id]) {
        startedCastaways[pick.user_id][pick.league_id] = {};
      }
      if (!startedCastaways[pick.user_id][pick.league_id][pick.episode_id]) {
        startedCastaways[pick.user_id][pick.league_id][pick.episode_id] = new Set();
      }
      startedCastaways[pick.user_id][pick.league_id][pick.episode_id].add(pick.castaway_id);
    });

    // Create user display name map
    const userNames: Record<string, string> = {};
    (rosters || []).forEach((r: any) => {
      if (r.users?.display_name) {
        userNames[r.user_id] = r.users.display_name;
      }
    });

    // Calculate bench points per user
    const userBenchPoints: Record<string, number> = {};

    Object.entries(userLeagueCastaways).forEach(([userId, leagues]) => {
      Object.entries(leagues).forEach(([leagueId, castaways]) => {
        Object.entries(castawayEpisodePoints).forEach(([episodeId, castawayPoints]) => {
          const started = startedCastaways[userId]?.[leagueId]?.[episodeId] || new Set();
          
          castaways.forEach((castawayId) => {
            if (!started.has(castawayId)) {
              // This castaway was on bench this episode
              const points = castawayPoints[castawayId] || 0;
              if (!userBenchPoints[userId]) {
                userBenchPoints[userId] = 0;
              }
              userBenchPoints[userId] += points;
            }
          });
        });
      });
    ...
    const leaderboard = Object.entries(userBenchPoints)
      .map(([userId, benchPoints]) => ({
        user_id: userId,
        display_name: userNames[userId] || 'Unknown',
        bench_points: Math.round(benchPoints * 10) / 10,
      }))
      .filter((u) => u.bench_points > 0)
