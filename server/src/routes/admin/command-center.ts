...
import { getSystemHealth, getDraftStats, getRecentActivity } from '../../services/admin-dashboard.js';
 ...
 */
router.get('/active-window', async (req: AuthenticatedRequest, res: Response) => {
  try {
    const now = DateTime.now().setZone('America/Los_Angeles');
    ...
    let mode: 'normal' | 'episode' | 'scoring' | 'draft' | 'incident' | 'off-cycle' = 'normal';
 ...
 */
export function broadcastToCommandCenter(event: { type: string; data: any }) {
...
interface AttentionItem {
  id: string;
  category: 'picks' | 'payments' | 'email' | 'scoring' | 'system' | 'drafts';
...
async function checkStripeHealth(): Promise<{ healthy: boolean; message: string }> {
  try {
...
async function checkJobHealth(): Promise<{ status: 'up' | 'degraded' | 'down'; failedCount: number; message: string }> {
  try {
...
function determineOverallStatus(services: Record<string, any>): 'operational' | 'degraded' | 'down' {
...
function generateWindowText(mode: string, data: Record<string, any>): { title: string; subtitle: string } {
  switch (mode) {
    case 'episode':
      return {
