...
router.get('/current', async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const leaguePickCounts: Record<string, { name: string; count: number }> = {};
    picksByLeague?.forEach(p => {
      const leagueId = p.league_id;
      if (!leaguePickCounts[leagueId]) {
        leaguePickCounts[leagueId] = { name: (p.leagues as any)?.name || 'Unknown', count: 0 };
      }
 ...
 */
router.get('/episode/:episodeId', async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    // Calculate stats
    const castawayPickCount: Record<string, { name: string; count: number; points: number }> = {};
    enrichedPicks?.forEach(p => {
      const cid = p.castaway_id;
      const cname = (p.castaways as any)?.name || 'Unknown';
...
router.get('/patterns', async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const leagueEpisodePicks: Record<string, Record<string, string[]>> = {};
    ...
    const accurateUsers = userScores?.filter((u: any) => 
      u.episodes_picked >= 5 && u.accuracy > 80
    ) || [];
 ...
 */
router.post('/send-reminders', async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (userIds && userIds.length > 0) {
      ...
      targetUsers = (allMembers || [])
        .filter(m => {
          ...
          return (m.users as any)?.email_notifications !== false;
        })
        .map(m => m.users)
        .filter(Boolean);
    }
    ...
    await logAdminAction(req, 'send_pick_reminders', 'episode', targetEpisodeId, {
      ...
      user_ids: targetUsers.slice(0, 100).map((u: any) => u.id),
    });
