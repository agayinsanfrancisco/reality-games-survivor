...
router.get('/active', async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const enrichedDrafts = (activeDrafts || []).map(draft => {
      ...
      const currentPicker = draft.league_members?.find((m: any) => m.user_id === currentPickerId);
...
router.get('/queue', async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const enrichedDrafts = (pendingDrafts || []).map(draft => ({
      ...
      code: draft.code,
      commissioner: draft.commissioner,
      memberCount: (draft.league_members as any)?.[0]?.count || 0,
      createdAt: draft.created_at,
      isReady: ((draft.league_members as any)?.[0]?.count || 0) >= 2,
    }));
 ...
 */
router.post('/:id/skip', async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { id } = req.params;
    const adminId = req.user!.id;
