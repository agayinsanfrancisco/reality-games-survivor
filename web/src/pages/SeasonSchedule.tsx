      ...
      <div className="bg-white rounded-2xl shadow-card p-4 border border-cream-200 mb-6">
        ...
        <div className="grid grid-cols-2 gap-3 text-sm">
          <div className="bg-cream-50 rounded-xl p-3 border border-cream-200">
            ...
            <p className="text-neutral-800 font-medium">{season?.premiere_at ? formatDate(season.premiere_at) : '-'}</p>
          </div>
          <div className="bg-cream-50 rounded-xl p-3 border border-cream-200">
            <p className="text-neutral-500">Finale</p>
            <p className="text-neutral-800 font-medium">{season?.finale_at ? formatDate(season.finale_at) : '-'}</p>
          </div>
          <div className="bg-cream-50 rounded-xl p-3 border border-cream-200">
            <p className="text-neutral-500">Draft Deadline</p>
            <p className="text-neutral-800 font-medium">{season?.draft_deadline ? formatDate(season.draft_deadline) : '-'}</p>
          </div>
          <div className="bg-cream-50 rounded-xl p-3 border border-cream-200">
            <p className="text-neutral-500">Registration Closes</p>
            <p className="text-neutral-800 font-medium">{season?.registration_closes_at ? formatDate(season.registration_closes_at) : '-'}</p>
          </div>
        </div>
      </div>
              ...
              {status === 'upcoming' && (
                <div className="mt-3 pt-3 border-t border-cream-200 text-sm">
                  <p className="text-neutral-500">
                    Picks lock: <span className="text-neutral-800">{formatDate(episode.picks_lock_at)} at {formatTime(episode.picks_lock_at)}</span>
                  </p>
                </div>
              )}
            </div>
