    ...
    <>
      ...
      <div className="bg-white rounded-2xl shadow-card border border-cream-200 mb-6">
        ...
        {episodes && episodes.length > 0 ? (
          ...
          <div className="p-8 text-center">
            ...
            <p className="text-neutral-400 text-sm">Results will appear after the first episode is scored.</p>
          </div>
        )}
      </div>
                      ...
                      {topThree.map((member: any, index: number) => (
                        <div
                          ...
                          className={`flex items-center gap-3 p-2 rounded-xl ${
                            index === 0 ? 'bg-burgundy-50 border border-burgundy-100' : 'bg-cream-50 border border-cream-200'
                          }`}
                        >
                          ...
                          <span className={`flex-1 ${index === 0 ? 'text-burgundy-600 font-medium' : 'text-neutral-800'}`}>
                            {member.users?.display_name}
                          </span>
                          <span className={`font-bold ${index === 0 ? 'text-burgundy-500' : 'text-neutral-500'}`}>
      ...
      <div className="mt-6 bg-burgundy-50 border border-burgundy-200 rounded-2xl p-4">
        ...
        <div className="grid grid-cols-3 gap-4 text-center">
          <div>
            <p className="text-2xl font-bold text-neutral-800">{(pastSeasons?.length || 0) + 1}</p>
