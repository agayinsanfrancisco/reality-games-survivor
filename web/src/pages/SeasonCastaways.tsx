...
import { ArrowLeft, Users, Trophy, Skull, Search, Loader2, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function SeasonCastaways() {
  ...
  const castawayTotals = allScores?.reduce((acc: Record<string, { total: number; byEpisode: Record<string, number> }>, score: any) => {
            ...
            >
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex items-center gap-2">
                    {scores?.total !== undefined && (
                      <span className={`text-lg font-bold ${scores.total >= 0 ? 'text-green-600' : 'text-red-600'}`}>
              ...
              {isExpanded && (
                <div className="px-4 pb-4 border-t border-cream-100">
                  ...
                  {/* Episode Scores */}
                  <div className="pt-3">
                    <h4 className="text-sm font-semibold text-neutral-700 mb-2">Episode Scores</h4>
                        ...
                        {Object.entries(scores.byEpisode)
                          ...
                          .map(([epNum, pts]) => (
                            <div key={epNum} className="text-center p-2 bg-cream-50 rounded-lg">
                              ...
                              <p className={`font-bold ${Number(pts) >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                                {Number(pts) >= 0 ? '+' : ''}{pts}
                              </p>
                            </div>
                          ))}
                  ...
                  {scores?.total !== undefined && (
                    <div className="mt-3 pt-3 border-t border-cream-100 flex justify-between items-center">
                      ...
                      <span className={`text-xl font-bold ${scores.total >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                        {scores.total >= 0 ? '+' : ''}{scores.total}
                      </span>
                    </div>
                  )}
