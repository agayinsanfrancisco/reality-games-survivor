...
Before suggesting ANY fix, change, or improvement, you MUST complete these steps in order:
...
5. **Verify deployed build matches the code you're reading**—check build logs, deployment timestamps, container images
...
One repo, one branch checked out, one source of truth. Worktrees create ghost state that causes exactly the "it works on my machine" bugs we're trying to kill.
...
This must be structural, not symptomatic. Ask yourself:
